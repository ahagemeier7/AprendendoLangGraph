...
#planejador simples divide em cálculo e explicação
def planner_node(state: SimpleAgentState)-> Dict[str,Any]:
  ...
  plan = [  
    ...
    },
    {
      ...
      "description": f"Explique em linguagem simples o resultado do cálculo feito na tarefa do 'do_math'."
    }
  ]
...
def mathematician_node(state: SimpleAgentState)-> Dict[str,str]:
  ...
  prompt = f"Você é um especialista em matemática. Resolva a expressão abaixo e forneça apenas o resultado numérico.\nExpressão: {desc}"
  
  try:
    response = models["gemini_2.5_flash"].invoke([HumanMessage(content=prompt)])
...
def writer_node(state: SimpleAgentState) -> Dict[str,str]:
  ...
  prompt = (
    f"Explique detalhadanente, passo a passom, como resolver a expressão abaixo, considerando a ordem das operações matemáticas.\n"
    f"Expressão: {original_query}\n"
    f"Resultado do cálculo: {math_result}\n"
  )
  try:
    response = models["gemini_2.5_flash"].invoke([HumanMessage(content=prompt)])
    return {"specialist_result": response.content.strip()}
  except Exception as e:
    return {"specialist_result": f"Erro ao escrever explicação: {e}"}

def collect_result_and_advance_node(state: SimpleAgentState) -> Dict[str,str]:
  current_taks_id = state.get("current_task_id")
  ...
  if current_taks_id:
    updated_intermediate_results[current_taks_id] = specialist_output
