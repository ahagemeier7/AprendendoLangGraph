...
def mathematician_node(state: SimpleAgentState)-> Dict[str,str]:
  ...
  if not match:
    return {"specialist_result": "Não foi possível identificar uma expressão matemática"}
...
def writer_node(state: SimpleAgentState) -> Dict[str,str]:
  ...
  return {"specialist_result": f"O resultado do cálculo é {math_result}. Isso significa que ao resolver a expressão chegamos a esse valor"}
