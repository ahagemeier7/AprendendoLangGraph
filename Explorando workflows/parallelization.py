...
def call_llm2(state: State):
  """Recebe o códigfo e retorna a analise do modelo gemini 1.5"""
  messages = [ 
    SystemMessage(content=SYSTEM_MESSAGE_LLMS.content),
    HumanMessage(content=f"Analise o seguinte código e forneca sugestões de melhorias: \n\n{state['query']}")
  ]
  response = models["gemini_1.5_flash"].invoke(messages)
...
coide_analysis_builder = StateGraph(State)

#adiciona os nós
coide_analysis_builder.add_node("call_llm1", call_llm1)
coide_analysis_builder.add_node("call_llm2", call_llm2)
coide_analysis_builder.add_node("judge", judge)

#Adiciona as arestas
coide_analysis_builder.add_edge(START, "call_llm1")
coide_analysis_builder.add_edge(START, "call_llm2")
coide_analysis_builder.add_edge("call_llm1", "judge")
coide_analysis_builder.add_edge("call_llm2", "judge")
coide_analysis_builder.add_edge("judge", END)

coide_analysis_workflow = coide_analysis_builder.compile()
